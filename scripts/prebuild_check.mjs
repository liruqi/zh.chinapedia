...
for (const f of files) {
  ...
  // ---- 4b. 相邻脚注标记之间的间距必须恰好一个空格 -------------------------
  // `[^16][^17]` 渲染成 `[16][17]`，两个上标挤在一起看不清；两个空格同样不规范
  // （仓库统一一个）。用前瞻匹配，链式标记 [^1][^2][^3] 才能逐处报出。
  // 定义行是 `[^n]:`，`]` 后跟 `:`，不会误命中；`[ \t]*` 不跨行，跨行的两行不误报。
  for (const m of body.matchAll(/\[\^[^\[\]]+\]([ \t]*)(?=\[\^[^\[\]]+\])/g)) {
    if (m[1] !== ' ') {
      // 前瞻匹配拿不到后面那个标记，从原文里截一段上下文来报
      const ctx = body.slice(m.index, m.index + 24).split('\n')[0];
      note(f, '脚注标记间距不规范', ctx + '  ← 应为恰好一个空格');
    }
  }
