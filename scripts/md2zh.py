#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""把英文维基 Markdown 条目译成简体中文 Markdown。
...
SYSTEM = """你是资深数学/百科译者，把英文维基百科条目译成简体中文。
...
def fix_footnotes(line, src):
    ...
    所以要逐行对账。
    """
    allow = collections.Counter(re.findall(r"\[\^(\d+)\]", src))
...
# 标题必须用专门的提示词，且只要一行。
HEADING_SYSTEM = ("把英文维基百科条目的小节标题译成简体中文。"
                  "只输出一行中文标题，前面照抄原文的 # 号个数。"
                  "不要解释，不要补充，不要另起段落。")


def _ask(cfg, system, text):
...
def main(argv=None):
    ap = argparse.ArgumentParser(description="英文维基 Markdown → 简体中文 Markdown")
    ap.add_argument("input", help="英文 .md 文件")
    ap.add_argument("-o", "--output", required=True, help="输出 .md 路径")
    ap.add_argument("--title", help="中文标题（不给就让模型译）")
    ...
    for start, items in chunks:
        ...
        for k, (idx, _) in enumerate(items):
            ...
            if re.match(r"^\[\^\d+\]:", line) and not re.match(r"^\[\^\d+\]:", src):
                line = ""                            # 模型自己编的脚注定义，丢掉
        ...
        if len(parts) > len(items):                  # 模型把一段拆成了多段
            ...
            keep = [l for l in parts[len(items):]
                    if not re.match(r"^\[\^\d+\]:", l) and not re.match(r"^#{1,6}\s", l)]
